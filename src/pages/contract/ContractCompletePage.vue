  ...
  <div class="max-w-screen min-h-screen mx-auto bg-yellow-50">
    ...
    <div v-if="currentStep === 'preview'" class="px-16 py-8">
      ...
      <div class="bg-white rounded-lg shadow-lg p-8">
        ...
        <div v-else-if="isLoading" class="mb-6 h-[600px] border border-gray-300 rounded-lg flex items-center justify-center">
        ...
        <div class="mt-6 p-4 bg-gray-100 rounded-lg">
          <label class="flex items-start">
            ...
            <span class="text-sm">
              ...
              법원에 소를 제기하기 전에 먼저 주택임대차분쟁조정위원회에 조정을 신청합니다.
            </span>
          </label>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-8 max-w-2xl mx-auto">
        <div class="text-center">
          ...
          <p class="text-gray-600 mb-2">
            <template v-if="exportStatus?.ownerSignatureCompleted && exportStatus?.buyerSignatureCompleted">
          ...
          <div v-if="exportStatus" class="mt-6 grid grid-cols-2 gap-4">
            <div class="p-3 border rounded-lg" :class="exportStatus.ownerSignatureCompleted ? 'bg-green-50 border-green-300' : 'bg-gray-50'">
              <p class="text-sm font-semibold">임대인</p>
              <p class="text-xs" :class="exportStatus.ownerSignatureCompleted ? 'text-green-600' : 'text-gray-500'">
                {{ exportStatus.ownerSignatureCompleted ? '✓ 서명 완료' : '대기 중...' }}
              </p>
            </div>
            <div class="p-3 border rounded-lg" :class="exportStatus.buyerSignatureCompleted ? 'bg-green-50 border-green-300' : 'bg-gray-50'">
              <p class="text-sm font-semibold">임차인</p>
              <p class="text-xs" :class="exportStatus.buyerSignatureCompleted ? 'text-green-600' : 'text-gray-500'">
...
watch(exportStatus, (newStatus) => {
  if (newStatus) {
    ...
    // 본인이 서명을 완료했는지 확인
    const mySignatureCompleted = userRole.value === 'owner' 
...
const setupWebSocket = async () => {
  try {
    ...
    setTimeout(() => {
      ...
      console.log('WebSocket 연결 상태 (재확인):', websocketService.getConnectionStatus())
...
const loadContractInfo = async () => {
  try {
    ...
    if (response.data && response.data.success) {
      ...
      } else if ((userRole.value === 'buyer' || userRole.value === 'tenant') && contractData.buyerSsn) {
...
const loadInitialPDF = async () => {
  ...
  try {
    ...
    const response = await createTempPdfUrl(contractId.value)
    
    if (response.success) {
      pdfUrl.value = response.data
      console.log('임시 PDF URL 생성 완료:', response.data)
    } else {
      throw new Error(response.message || '임시 URL 생성에 실패했습니다')
    }
  } catch (error) {
...
const isSignatureComplete = computed(() => {
  ...
  const hasRequiredSignatures = (!hasTaxArrears.value || !!signatures.value.signature2) &&
...
const isPasswordValid = computed(() => {
  return contractPassword.value.length >= 4 && 
         contractPassword.value === contractPasswordConfirm.value
})
...
const proceedToPassword = async () => {
  ...
  try {
    ...
    // STOMP 연결이 완전히 준비될 때까지 대기
    let retryCount = 0
    const maxRetries = 10
    while (retryCount < maxRetries) {
      const stompClient = websocketService.getStompClient()
      const stompState = stompClient?.state
      const isStompConnected = stompClient?.connected
      console.log(`STOMP 연결 확인 시도 ${retryCount + 1}: state=${stompState}, connected=${isStompConnected}`)
      
      if (stompState === 1 && isStompConnected) { // 1 = CONNECTED
        console.log('STOMP 연결 준비 완료')
        break
      }
    ...
    if (retryCount === maxRetries) {
      console.error('STOMP 연결 대기 시간 초과')
    }
    ...
    // WebSocket 서비스 메서드 사용
    const sendResult = websocketService.sendContractExportSignature(contractId.value, signatureMessage)
...
const saveSignatureToServer = async () => {
  try {
    ...
    if (signatures.value.signature1) {
      const blob1 = signatures.value.signature1.blob || 
    ...
    if (signatures.value.signature2) {
      const blob2 = signatures.value.signature2.blob || 
    ...
    if (signatures.value.signature3) {
      const blob3 = signatures.value.signature3.blob || 
